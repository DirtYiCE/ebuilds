...
EAPI=6
inherit xdg-utils nsplugins multilib multilib-minimal unpacker
...
SRC_URI="
	nsplugin? (
		abi_x86_64? ( https://archlinux.org/packages/extra/x86_64/flashplugin/download/ -> ${P}-npapi.x86_64.tar.zst )
	)
	ppapi? (
		abi_x86_64? ( https://archlinux.org/packages/extra/x86_64/pepper-flash/download/ -> ${P}-ppapi.x86_64.tar.zst )
	)
"
SLOT="22"
...
multilib_src_install() {
	...
	if use nsplugin; then
		...
		"${FILESDIR}/binpatch.rb" usr/lib/mozilla/plugins/libflashplayer.so < "${FILESDIR}/flash.bin" > libflashplayer.so || die
	...
	if use ppapi; then
		...
		"${FILESDIR}/binpatch.rb" usr/lib/PepperFlash/libpepflashplayer.so < "${FILESDIR}/flash.bin" > libpepflashplayer.so || die
		doexe libpepflashplayer.so
		insinto /usr/$(get_libdir)/chromium/PepperFlash
		doins usr/lib/PepperFlash/manifest.json
