...
UIM_PATCH_REV="0.6.16.2"
UIM_PATCH_URL="https://github.com/e-kato/macuim/archive/v${UIM_PATCH_REV}.tar.gz"
...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="emacs fcitx uim ibus +qt4 renderer test"
...
src_unpack() {
	...
	use uim && unpack $(basename ${UIM_PATCH_URL})
...
src_prepare() {
	...
	if use uim; then
		cp -rf ../macuim-${UIM_PATCH_REV}/Mozc/uim unix/ || die
	fi
	epatch_user
}
...
src_install() {
	...
	if use uim ; then
		...
		doins "../macuim-${UIM_PATCH_REV}/Mozc/scm/"*

		insinto /usr/share/uim/pixmaps
		pushd data/images/unix >/dev/null
		newins ime_product_icon_opensource-32.png mozc.png
		newins ui-tool.png mozc_tool_selector.png
		newins ui-properties.png mozc_tool_config_dialog.png
		newins ui-dictionary.png mozc_tool_dictionary_tool.png
		popd >/dev/null
	fi
