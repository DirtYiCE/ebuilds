...
RDEPEND="${PYTHON_DEPS}
	...
	>=dev-python/PySocks-1.6.8
	>=dev-python/gevent-20.9.0[test]
...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="https://github.com/zeronet-conservancy/zeronet-conservancy/archive/v${PV}.tar.gz -> ${P}.tar.gz"
	KEYWORDS="~amd64 ~x86"
	S="${WORKDIR}/zeronet-conservancy-${PV}"
fi
...
"
	...
	python_domodule ${PN}.py plugins src
