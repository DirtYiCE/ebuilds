...
KEYWORDS="~x86 amd64"
DESCRIPTION="Interlink Mail & News"
...
src_configure() {
	# Basic configuration:
	mozconfig_init projects/mail
	# restore jetpack, some addons need it
	mozconfig_var MOZ_JETPACK 1
	sed -i '/MOZ_JETPACK=/d' projects/mail/confvars.sh || die
	...
	# the built-in ones:
	append-ldflags -Wl,-rpath="$EPREFIX/usr/$(get_libdir)/${PN}"
...
interlink_install_branding_files() {
...
src_install() {
	...
	tar xjpf "${S}/${obj_dir}/dist/${PN}-${PACKAGE_V}.linux-${CTARGET_default%%-*}.tar.bz2" || die
	einfo "Installing the package..."
	local dest_libdir="/usr/$(get_libdir)"
	mkdir -p "${D}/${dest_libdir}" || die
	...
	pax-mark m "${D}/${dest_libdir}/${PN}/"{interlink,interlink-bin,plugin-container}
