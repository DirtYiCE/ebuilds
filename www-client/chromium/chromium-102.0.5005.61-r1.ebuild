...
IUSE="component-build cups cpu_flags_arm_neon debug +gtk2 gtk3 gtk4 +hangouts headless +js-type-check kerberos libcxx offensive +official pic +proprietary-codecs pulseaudio screencast selinux +suid +system-ffmpeg +system-harfbuzz +system-icu +system-png ungoogled user-select vaapi wayland widevine"
REQUIRED_USE="
...
src_prepare() {
	...
	use offensive && PATCHES+=("${FILESDIR}/chromium-100-offensive.patch")
	...
	if use ungoogled; then
		...
		if use js-type-check; then
			grep -v 'third_party/closure_compiler' "${src}"/pruning.list
		else
