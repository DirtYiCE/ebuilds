...
PATCHES=(
	...
	"${FILESDIR}/chromium-69-compile.patch"
)
...
debian/disable/swiftshader.patch
debian/warnings/attribute.patch
debian/warnings/enum-compare.patch
debian/warnings/initialization.patch
debian/warnings/multichar.patch
debian/warnings/null-destination.patch
debian/warnings/printf.patch
debian/warnings/sequence-point.patch
debian/warnings/unused-typedefs.patch
...
ungoogled-chromium/add-third-party-ungoogled.patch
...
ungoogled-chromium/add-flag-to-enable-potentially-annoying-security-features.patch
...
src_prepare() {
	...
	cp "${FILESDIR}/log_list.json" components/certificate_transparency/data/log_list.json || die

	if use ungoogled; then
		local src="${WORKDIR}/ungoogled-chromium-${UNGOOGLED_PV}"
		local cfg="${src}/config_bundles/common"
		einfo "ungoogled-chromium: cleaning"
		xargs -d'\n' rm < "${cfg}"/pruning.list || die

		einfo "ungoogled-chromium: patching"
		for patch in "${UNGOOGLED_PATCHES[@]}"; do
			epatch -p1 "${src}/patches/${patch}"
		done

		einfo "ungoogled-chromium: substituting domains"
		sed -Ee 's/^(.*)#(.*)$/s#\1#\2#g/' -e 's/\\g<([0-9])>/\\\1/g' \
			"${cfg}"/domain_regex.list > "${cfg}"/domain_regex.sed || die
		xargs -d'\n' sed -Ei -f "${cfg}"/domain_regex.sed \
			< "${cfg}"/domain_substitution.list || die
	...
	local keeplibs=(
		...
		tools/gn/base/third_party/icu # needed for gn bootstrap
	...
	if ! use ungoogled; then
		keeplibs+=( third_party/analytics )
	fi
...
bootstrap_gn() {
	...
	einfo "Building GN..."
	set -- tools/gn/bootstrap/bootstrap.py -s -v
	echo "$@"
	"$@" || die
}
